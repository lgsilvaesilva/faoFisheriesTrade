# XXX: this should be in a datatable
# output is data.table with `reporter` and `year` column

generate_regions <- function() {
  m49_codes <-
    tibble::tribble(
      ~country, ~m49, ~iso,
      "Afghanistan", "004", "AFG",
      ...
      "Zambia", "894", "ZMB",
      "Zimbabwe", "716", "ZWE",
      ### XXX the following are not part of the original
      # table at https://unstats.un.org/unsd/methodology/m49/
      # but were found on SWS.
      "China,Taiwan", "158", NA,
      "Sudan (former)", "736", NA,
      "NethAntilles", "530", NA
    ) %>%
    mutate(
      m49    = as.character(as.integer(m49)),
      region = countrycode::countrycode(iso, "iso3c", "region"),
      # For those found on SWS:
      region = ifelse(m49 == "158", "Eastern Asia", region),
      region = ifelse(m49 == "736", "Northern Africa", region),
      region = ifelse(m49 == "530", "Caribbean", region)
      # 736 is Former Sudan, leaving it unassigned.
    ) %>%
    mutate(m49 = as.integer(m49)) %>%
    select(country, reporter = m49, region)
