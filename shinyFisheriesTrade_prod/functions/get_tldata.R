...
get_tldata <- function(.reporter, .years, .flow) {
  ...
  hs_filter_sql <- paste0("comm SIMILAR TO '", substr(hs_filter_sql, 1, nchar(hs_filter_sql) - 1), "'")

  ##-- Trade table name
  trade_table_name <- "ct_tariffline_unlogged_"
  ...
  ##-- Filter reporter ----
  if(.reporter != "all") {
    rep_sql <- sprintf("rep IN (%s)", toString(sprintf("'%s'", .reporter)))
  ...
  sql_columns <- c("tyear", "rep", "flow", "comm", "hsrep", "prt", "weight", "qty", "qunit", "tvalue")
  ...
  tradedata <- tradedata[!grepl(hs_exclude_regex, comm), ]
